<script lang="ts">
	...
	async function recordDeviation(description: string, justification: string) {
		await executionStore.act({ action: 'record_deviation', description, justification });
		showDeviationDialog = false;
	}

	async function addStep(heading: string, description?: string, afterStep?: string) {
		await executionStore.act({ action: 'add_step', heading, description, after_step: afterStep });
	...
	async function abortExecution() {
		...
		await executionStore.act({ action: 'abort', reason: abortReason.trim() });
...
<div class="execution-page">
	...
	{:else if summary}
		...
		{#if isActive}
			<div class="toolbar">
				<button class="btn btn-secondary" onclick={() => (showAddStepDialog = true)}>
					+ Add Step
				</button>
				<button class="btn btn-secondary" onclick={() => (showDeviationDialog = true)}>
					Record Deviation
				</button>
				<div class="toolbar-spacer"></div>
				<button class="btn btn-success" onclick={() => (showCompleteDialog = true)}>
					Complete
				</button>
				<button class="btn btn-danger" onclick={() => (showAbortDialog = true)}>
					Abort
				</button>
			</div>
		{/if}
		...
		{#if isFinished}
			<div class="finish-banner" class:pass={summary.status === 'pass'} class:fail={summary.status === 'fail'} class:aborted={summary.status === 'aborted'}>
				Execution {summary.status === 'pass' ? 'passed' : summary.status === 'fail' ? 'failed' : 'aborted'}
...
{#if showCompleteDialog}
	<div class="modal-backdrop" role="presentation" onclick={() => (showCompleteDialog = false)}>
		...
		<div class="modal" onclick={(e) => e.stopPropagation()}>
			...
			<div class="modal-actions">
				<button class="btn btn-secondary" onclick={() => (showCompleteDialog = false)}>Cancel</button>
				<button class="btn btn-danger" onclick={() => completeExecution('fail')}>Fail</button>
				<button class="btn btn-success" onclick={() => completeExecution('pass')}>Pass</button>
			</div>
		</div>
	</div>
{/if}
...
{#if showAbortDialog}
	<div class="modal-backdrop" role="presentation" onclick={() => { showAbortDialog = false; abortReason = ''; }}>
		<!-- svelte-ignore a11y_click_events_have_key_events -->
		<!-- svelte-ignore a11y_no_static_element_interactions -->
		<div class="modal" onclick={(e) => e.stopPropagation()}>
			...
			<div class="modal-actions">
				<button class="btn btn-secondary" onclick={() => { showAbortDialog = false; abortReason = ''; }}>Cancel</button>
				<button class="btn btn-danger" onclick={abortExecution} disabled={!abortReason.trim()}>Abort</button>
			</div>
		</div>
	</div>
{/if}
