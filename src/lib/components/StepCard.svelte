<script lang="ts">
	...
	function completeStep() {
		onaction({ action: 'complete_step', step_heading: stepSummary.heading });
	}

	function confirmSkip() {
		if (!skipReason.trim()) return;
		onaction({ action: 'skip_step', step_heading: stepSummary.heading, reason: skipReason.trim() });
		showSkipDialog = false;
		skipReason = '';
	}

	function toggleCheckbox(text: string, checked: boolean) {
		onaction({ action: 'toggle_checkbox', step_heading: stepSummary.heading, text, checked });
	}

	function recordInput(label: string, value: string, unit?: string) {
		onaction({ action: 'record_input', step_heading: stepSummary.heading, label, value, unit });
	}
...
>
	...
	{#if executionActive}
		<div class="step-actions">
			{#if isPending}
				<button class="btn btn-primary" onclick={startStep}>Start Step</button>
				<button class="btn btn-muted" onclick={() => (showSkipDialog = true)}>Skip</button>
			{:else if isActive}
				<button class="btn btn-primary" onclick={completeStep}>Complete Step</button>
				<button class="btn btn-muted" onclick={() => (showSkipDialog = true)}>Skip</button>
			{/if}
		</div>
	{/if}
	...
	{#if showSkipDialog}
		<div class="skip-dialog">
			...
			<div class="skip-actions">
				<button class="btn btn-muted" onclick={() => { showSkipDialog = false; skipReason = ''; }}>Cancel</button>
				<button class="btn btn-warn" onclick={confirmSkip} disabled={!skipReason.trim()}>Skip Step</button>
			</div>
		</div>
	{/if}
</div>
